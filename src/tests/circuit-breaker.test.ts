...
describe('CircuitBreaker', () => {
  ...
  it('opens circuit after threshold failures', async () => {
    ...
    for (let i = 0; i < 3; i++) {
      try { await withCircuitBreaker('scope-b', fail); } catch { /* expected failure */ }
    }
  ...
  it('resets failures after success', async () => {
    ...
    for (let i = 0; i < 2; i++) {
      try { await withCircuitBreaker('scope-c', fail); } catch { /* expected failure */ }
    }
  ...
  it('carries scope on CircuitOpenError', async () => {
    const fail = async () => { throw new Error('boom'); };
    for (let i = 0; i < 3; i++) {
      try { await withCircuitBreaker('scope-d', fail); } catch { /* expected failure */ }
    }
